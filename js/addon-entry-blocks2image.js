...
/* harmony default export */ __webpack_exports__["default"] = (async function (_ref) {
  ...
  async function exportBlock(isExportPNG, block) {
    ...
    console.log("Processing ".concat(imageElements.length, " images for export..."));
    ...
    console.log("Found ".concat(urlCounts.size, " unique image URLs among ").concat(imageElements.length, " total images"));
    ...
    for (let i = 0; i < imageElements.length; i++) {
      ...
      if (imageCache.has(iconUrl)) {
        ...
        if (i % 100 === 0) {
          console.log("Processing image ".concat(i + 1, "/").concat(imageElements.length, ": ").concat(iconUrl, " (cached)"));
        }
        continue;
      }
      try {
        console.log("Processing image ".concat(i + 1, "/").concat(imageElements.length, ": ").concat(iconUrl, " (fetching...)"));
        uniqueImages++;
        ...
        if (blob.size > 1024 * 1024) {
          // 1MB limit per image
          console.warn("Skipping large image (".concat((blob.size / 1024 / 1024).toFixed(1), "MB): ").concat(iconUrl));
          continue;
        }
        ...
        console.warn("Failed to process image ".concat(iconUrl, ":"), error.message);
    ...
    console.log("Image processing complete! Fetched ".concat(uniqueImages, " unique images, used cache for ").concat(cacheHits, " duplicates"));
    console.log("Cache efficiency: ".concat(cacheHits > 0 ? (cacheHits / (uniqueImages + cacheHits) * 100).toFixed(1) : 0, "% cache hit rate"));
    if (!isExportPNG) {
      exportData(svg);
    } else {
      exportPNG(svg);
  ...
  function exportData(svg) {
    ...
    svg.setAttribute("width", "".concat(svgBounds.width, "px"));
    svg.setAttribute("height", "".concat(svgBounds.height, "px"));
    svg.setAttribute("viewBox", "0 0 ".concat(svgBounds.width, " ").concat(svgBounds.height));
    ...
    const data = new Blob([updatedSvgString], {
      type: "image/svg+xml"
    });
  ...
  function exportPNG(svg) {
    ...
    try {
      ...
      if (width > 8000 || height > 8000) {
        const userConfirm = confirm("This block stack is very large (".concat(Math.round(width), "\xD7").concat(Math.round(height), " pixels). ") + "Exporting as PNG may use a lot of memory and could crash your browser. " + "Consider exporting as SVG instead, or click OK to continue with PNG export.");
        if (!userConfirm) {
          iframe.remove();
          return;
        }
      ...
      function createCanvasWithMemoryCheck(targetWidth, targetHeight, initialScale) {
        ...
        while (attempts < maxAttempts) {
          try {
            ...
            console.log("Attempt ".concat(attempts + 1, ": Canvas ").concat(canvasWidth, "\xD7").concat(canvasHeight, ", estimated memory: ").concat(estimatedMemoryMB.toFixed(1), "MB, scale: ").concat(currentScale.toFixed(2)));
            ...
            console.log("Successfully created canvas with scale factor: ".concat(currentScale.toFixed(2)));
            break;
          } catch (error) {
            console.warn("Canvas creation failed at scale ".concat(currentScale.toFixed(2), ":"), error.message);
      ...
      img.onload = function () {
        try {
          ...
          link.download = "block_".concat(timestamp, ".").concat(fileExtension);
          link.href = dataURL;
          link.click();

          // Log success with details
          if (finalScale < scaleFactor * 0.9) {
            console.log("Export successful! Image was automatically scaled to ".concat((finalScale / scaleFactor * 100).toFixed(0), "% to prevent memory issues."));
          } else {

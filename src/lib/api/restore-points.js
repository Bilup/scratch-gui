...
/**
 * Everything a restore point needs from the project that is currently in the VM.
 *
 * Capturing and writing are separate so that a caller about to replace the
 * project can take the snapshot first and write it later. The snapshot has to be
 * taken before the VM changes, but nothing is waiting on the database, and the
 * write is the slower half: one row per asset, then a scan of the whole store.
 *
 * @typedef {object} RestorePointCapture
 * @property {VirtualMachine} vm the VM the snapshot was taken from
 * @property {string} title
 * @property {MetadataType} type
 * @property {Record<string, Uint8Array>} projectFiles
 * @property {Uint8Array} jsonData contents of project.json
 * @property {string[]} projectAssetIDs asset file names, excluding project.json
 * @property {{type: string, data: ArrayBuffer}} thumbnailData
...
/**
 * Take a snapshot of the project currently in the VM. Performs no database work.
 * @param {VirtualMachine} vm scratch-vm instance
 * @param {string} title project title
 * @param {MetadataType} type restore point type
 * @returns {Promise<RestorePointCapture>} rejects when the project has no assets
 */
const captureRestorePoint = (
    ...
    if (projectAssetIDs.length === 0) {
        return Promise.reject(new Error('There are no assets in this project'));
    }

    // Signal start of restore point creation (pauses sync)
    vm.emit('RESTORE_POINT_START');

    return generateThumbnail(vm).then(thumbnailData => ({
        vm,
        title,
        type,
        projectFiles,
        jsonData,
        projectAssetIDs,
        // Renderer unavailable; a 1x1 transparent PNG keeps the restore point
        // valid and non-zero-sized.
        ...
        stageWidth: vm.runtime.stageWidth,
        stageHeight: vm.runtime.stageHeight,
        created: Math.round(Date.now() / 1000)
    }));
};

/**
 * Write a captured snapshot to the database.
 * @param {RestorePointCapture} capture A snapshot from captureRestorePoint().
 * @returns {Promise<void>} resolves when the restore point has been written
 */
const writeRestorePoint = capture => openDB().then(db => new Promise((resolveTransaction, rejectTransaction) => {
    const {
        vm,
        title,
        type,
    ...
    // Every request is issued up front instead of being awaited one at a time.
    // They all belong to this one transaction, so IndexedDB serialises them
    // regardless, but awaiting each in turn adds a round trip per asset to a
    // project that routinely has hundreds of them.
    const writeMissingAssets = missingAssets => {
        const assetStore = transaction.objectStore(ASSET_STORE);
        if (missingAssets.length === 0) {
            writeThumbnail();
            return;
        }
        const writeAsset = assetId => new Promise(resolveAsset => {
            // TODO: should we insert arraybuffer or uint8array?
            const request = assetStore.add(projectFiles[assetId], assetId);
            request.onsuccess = () => resolveAsset();
        });
        // A request that fails aborts the transaction, which rejects the restore
        // point through transaction.onerror, so there is no rejection path here.
        Promise.all(missingAssets.map(writeAsset)).then(writeThumbnail);
    };
...
/**
 * Snapshot the current project and write it out as a restore point.
 * @param {VirtualMachine} vm scratch-vm instance
 * @param {string} title project title
 * @param {MetadataType} type restore point type
 * @returns {Promise<void>} resolves when the restore point is created
 */
const createRestorePoint = (vm, title, type) => openDB()
    .then(() => captureRestorePoint(vm, title, type))
    .then(writeRestorePoint);

const warnRestorePointFailure = error => {
    // eslint-disable-next-line no-console
    console.warn('Could not create safety restore point', error);
};

/**
 * Take a safety snapshot without writing it yet.
 *
 * Callers that are about to replace the project should use this instead of
 * createSafetyRestorePoint(), so that the database half of the work stays off
 * their critical path, and hand the result to writeSafetyRestorePoint() once the
 * new project is in place.
 *
 * @param {VirtualMachine} vm scratch-vm instance
 * @param {string} title project title
 * @returns {Promise<?RestorePointCapture>} the snapshot, or null if it could not be taken
 */
const captureSafetyRestorePoint = (vm, title) => Promise.resolve()
    .then(() => captureRestorePoint(vm, title || '?', TYPE_AUTOMATIC))
    .catch(error => {
        warnRestorePointFailure(error);
        return null;
    });

/**
 * Write a snapshot taken by captureSafetyRestorePoint() and prune old entries.
 * @param {?RestorePointCapture} capture A snapshot, or null to do nothing.
 * @returns {Promise<void>} resolves once the write has been attempted; a failure
 * is warned about rather than thrown, matching createSafetyRestorePoint()
 */
const writeSafetyRestorePoint = capture => {
    if (!capture) return Promise.resolve();
    return Promise.resolve()
        .then(() => writeRestorePoint(capture))
        .then(() => removeExtraneousRestorePoints())
        .catch(warnRestorePointFailure);
};

const createSafetyRestorePoint = (vm, title) => captureSafetyRestorePoint(vm, title)
    .then(writeSafetyRestorePoint);
...
export default {
    ...
    // For callers that are about to replace the project: take the snapshot, then
    // write it once the database work is no longer in the way.
    captureSafetyRestorePoint,
    writeSafetyRestorePoint,
